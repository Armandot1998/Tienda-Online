...
class ControladorFactura extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO Factura (idFactura,fecha,idDetalleDeFactura,UsuariosidUsuarios) VALUES (?,?,?,?);";
      $parametros = array($factura->idFactura,$factura->fecha,$factura->idDetalleDeFactura,$factura->UsuariosidUsuarios);
   ...
   {
      $parametros = array($factura->idFactura,$factura->fecha,$factura->idDetalleDeFactura,$factura->UsuariosidUsuarios,$factura->id);
      $sql = "UPDATE Factura SET idFactura = ?,fecha = ?,idDetalleDeFactura = ?,UsuariosidUsuarios = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
